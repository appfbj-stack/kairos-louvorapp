...
import { ArrowLeft, Music, Youtube, Link, AlignLeft, Check, Save, Cloud, ListChecks, ExternalLink } from 'lucide-react';
...
const EditSong: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (song) {
      setFormData({ ...song, arrangement: song.arrangement || '' });
    } else {
      navigate('/repertorio');
    }
  }, [id, songs, navigate]);
  ...
  return (
    <div className="max-w-5xl mx-auto pb-20 animate-in fade-in slide-in-from-right-4 duration-500">
      <div className="mb-8 flex items-center gap-4">
        ...
        <div>
          <h2 className="text-3xl font-serif font-bold text-slate-900">Editar Música</h2>
          <p className="text-slate-500 text-sm">Atualize dados, cifras e arranjo. As alterações ficam salvas para todos os músicos.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        <section className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-400 flex items-center gap-1">
                <Link size={12} className="text-slate-500" /> Link da Cifra
              </label>
              <div className="relative">
          ...
          {(formData.youtubeUrl || formData.audioUrl) && (
            <div className="pt-4 border-t border-slate-100 space-y-2">
              ...
              <ReferencePlayer
                ...
                title={formData.title || 'Música em Edição'}
